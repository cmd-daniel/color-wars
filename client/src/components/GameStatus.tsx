...
const GameStatus = (/*{ onLeave }: GameStatusProps*/) => {
  ...
  const takenIcons = Object.values(players).map((p) => p.icon).filter(Boolean);
  ...
  const handleStartGame = () => {
    ...
    const playersWithoutIcon = Object.values(players).filter((p) => !p.icon);
  ...
  return (
    <div className="w-full flex justify-center p-4">
        <ul className="flex w-full max-w-[420px] flex-col gap-2 bg-secondary p-4 rounded-lg">
                ...
                <div className="flex items-center h-full gap-3">
                  ...
                  >
                    <PopoverTrigger asChild>
                      <span
                        className={`
                          h-full self-stretch  w-3 rounded-[2px]  border
                      ...
                      <PopoverContent asChild>
                        <div className="grid grid-cols-5 gap-2 p-2">
                          {availableColors.map((color) => (
                            <button
                              key={color}
                              className="h-6 w-6 rounded-full border hover:scale-110 transition"
