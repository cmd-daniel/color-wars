...
export class TerrainMesh extends PIXI.Container {
  ...
  init(hexes: AxialHex[], hexSize: number, texture: PIXI.Texture) {
    ...
    let vPtr = 0, iPtr = 0, vertexCount = 0;
    ...
    for (let i = 0; i < totalHexes; i++) {
      ...
      // Verts (TL, TR, BR, BL)
      verts[vPtr++] = cx - w2; verts[vPtr++] = cy - h2;
      verts[vPtr++] = cx + w2; verts[vPtr++] = cy - h2;
      verts[vPtr++] = cx + w2; verts[vPtr++] = cy + h2;
      verts[vPtr++] = cx - w2; verts[vPtr++] = cy + h2;
      ...
      indices[iPtr++] = offset + 0; indices[iPtr++] = offset + 1; indices[iPtr++] = offset + 2;
      indices[iPtr++] = offset + 0; indices[iPtr++] = offset + 2; indices[iPtr++] = offset + 3;
      vertexCount += 4;
