...
const computeInteriorBox = (polygon: Point[], center: Point, padding: number) => {
  ...
  if (
    ![right, left, up, down].every((distance) => Number.isFinite(distance) && distance > EPSILON)
  ) {
    return null;
  }
...
const computeLabelMetrics = (
  ...
  const approximateWidth = Math.max(
    ...
    Math.sqrt(Math.max(polygonAreaValue, 1)),
  );
  ...
  const longestWord =
    words.reduce((max, word) => Math.max(max, word.length), 0) || sanitized.length || 1;
  ...
  while (iterations < 3) {
    const charsPerLine = Math.max(
      longestWord,
      Math.floor(usableWidth / (Math.max(fontSize, EPSILON) * charWidthFactor)) || 1,
    );
