...
const PaintControls = () => {
  ...
  return (
    <section className="panel">
      ...
      {interactionMode === "view" && (
        <p className="panel__placeholder">Switch to edit mode to modify the map.</p>
      )}
      {interactionMode === "edit" &&
        paintMode !== "erase" &&
        paintMode !== "delete-hex" &&
        !selectedStateId && (
          <p className="panel__placeholder">
            Select a territory in the sidebar to enable painting.
          </p>
        )}
      {paintMode === "flood" && (
        <p className="panel__helper">
          Flood fill paints all connected hexes that currently share the clicked hex&apos;s original
          owner.
        </p>
      )}
      {paintMode === "delete-hex" && interactionMode === "edit" && (
        <p className="panel__helper">
          Delete removes the hex entirely from the grid and its state.
        </p>
      )}
    </section>
  );
