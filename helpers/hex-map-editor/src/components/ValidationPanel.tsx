...
const ValidationPanel = () => {
  ...
  return (
    <section className="panel panel--validation">
      <header>
        <h3>Validation</h3>
        <span className="tag tag--alert">{sortedIssues.length} issue{sortedIssues.length > 1 ? 's' : ''}</span>
      </header>
          ...
          return (
            ...
            >
              ...
              {issue.hexKeys && issue.hexKeys.length > 0 && (
                <p className="validation-item__extra">Examples: {issue.hexKeys.join(', ')}{issue.hexKeys.length >= 12 ? '…' : ''}</p>
              )}
            </li>
          )
        })}
