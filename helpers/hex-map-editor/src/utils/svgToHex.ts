...
const buildPathMap = (paths: SvgPathDefinition[]) =>
  paths
    ...
    .map((path, index) => {
      const fallbackColor =
        path.fill && path.fill !== "transparent" ? path.fill : `hsl(${(index * 47) % 360} 65% 55%)`;
...
const interpolateHexes = (svg: ParsedSvgDocument, overlay: GridOverlayConfig): RasterisedResult => {
  ...
  const corners = [
    ...
    pixelToAxial(
      svg.viewBox.x + svg.viewBox.width - offsetX,
      svg.viewBox.y + svg.viewBox.height - offsetY,
      grid,
    ),
  ];
    ...
    for (let q = minQ; q <= maxQ; q += 1) {
      ...
      for (const path of pathEntries) {
        ...
        if (
          !winnerState &&
          path.stroke &&
          context.isPointInStroke(path.path2d, sampledX, sampledY)
        ) {
          winnerState = path.stateId;
          if (!colorToState.has(path.colorKey)) {
            colorToState.set(path.colorKey, path.stateId);
  ...
  if (!hexes.length) {
    console.warn(
      "[SVG→Hex] No hexes produced. Check if the SVG paths are closed/fillable or adjust sampling parameters.",
    );
